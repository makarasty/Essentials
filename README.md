...
## Modular builds
~~This readme section generated by AI~~

Use `-PexcludeModules` to compile an artifact without optional services.<br>
The matching Kotlin source, generated code, resources, and module-specific libraries are excluded from both `shadowJar` and `proguardJar`.
...
# Shared-database server that must not execute or package Flyway migrations
...
`discord`, `effect`, `migration`, `protect`, `vote`, and `web`. `achievement`
and `flyway` are accepted as aliases<br>
`services` (also `core/services`) expands to every optional module. A build without `migration` keeps R2DBC/Exposed database access but skips Flyway and omits its PostgreSQL/MariaDB JDBC drivers.
...
The full test suite targets the full distribution.<br>
For a modular build, the `test` task runs a headless Mindustry boot smoke test against the exact sources packaged in that artifact.